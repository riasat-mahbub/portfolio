...
All three social link `<a>` tags in social-links.astro had `border border-1` in their class attribute. The Tailwind `border-1` utility does not exist — `border` already sets a 1px width. The `border-1` class was a no-op and a code smell.
