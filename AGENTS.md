...
│   └── BlogPost.astro           # Post wrapper: title, tags, difficulty, reading time, prev/next
...
├── content/
│   ├── config.ts                # Zod schema: title, description, difficulty, tags, publishDate
...
- **Blog posts**: MDX in `src/content/blog/`. Frontmatter must match the Zod schema in `content/config.ts`. Tags are free-form strings. Use `getCollection('blog')` from `astro:content` to query.
- **Package manager**: CI uses npm. Repo contains both `package-lock.json` and `pnpm-lock.yaml`.
- **No tests** — no test runner configured yet.

## Notes
- Firebase is a dependency but not yet wired up in source code.
- `tracker/` directory contains a file-based project knowledge graph (bugs, features, decisions, epics, tasks).
- Prettier available as dev dependency; no explicit config file (uses defaults).
